...
type PlayerView = 'module' | 'qg' | 'coaching';

const CoursePlayer: React.FC<CoursePlayerProps> = ({ modules, onBack, onLogout }) => {
  const [activeModuleId, setActiveModuleId] = useState<string>(modules[0].id);
  const [currentView, setCurrentView] = useState<PlayerView>('module');
  ...
  const renderModuleContent = () => (
    <>
      ...
      <div className="animate-in fade-in slide-in-from-bottom-12 duration-1000">
        ...
        {/* Section Instruments (ex-Ressources) en bas de module */}
    ...
    <div className="flex h-screen bg-black text-white overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-80 bg-neutral-950 border-r border-white/5 flex flex-col transition-transform duration-500 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 shadow-2xl`}>
        ...
        <div className="flex-1 overflow-y-auto py-8 scrollbar-hide px-6">
          ...
          {/* 2. SECTION QG (ARSENAL) */}
          <div className="mb-4 text-[10px] font-black text-gray-700 uppercase tracking-widest px-2">Quartier Général</div>
          <div className="space-y-1 mb-8">
            <button
              onClick={() => setCurrentView('qg')}
              className={`w-full p-4 flex items-center gap-4 transition-all rounded-2xl relative ${currentView === 'qg' ? 'bg-[#ff0000]/10 border border-[#ff0000]/20' : 'hover:bg-white/[0.03] border border-transparent'}`}
            >
              <div className={`${currentView === 'qg' ? 'text-[#ff0000]' : 'text-gray-700'}`}>
                <Shield size={18} />
              </div>
              <div className="text-left">
                <div className={`text-[11px] font-black uppercase tracking-tight ${currentView === 'qg' ? 'text-white' : 'text-gray-500'}`}>QG & Groupe Privé</div>
                <div className="text-[9px] text-gray-700 font-bold uppercase italic">Arsenal & Outils</div>
              </div>
              {currentView === 'qg' && <div className="ml-auto w-1.5 h-1.5 bg-[#ff0000] rounded-full" />}
            </button>
          </div>

          {/* 3. SECTION MENTOR (COACHING) */}
          ...
          <div className="space-y-1 mb-12">
            <button
              ...
              className={`w-full p-4 flex items-center gap-4 transition-all rounded-2xl relative ${currentView === 'coaching' ? 'bg-[#ff0000]/10 border border-[#f4c024]/20' : 'hover:bg-white/[0.03] border border-transparent'}`}
            >
              <div className={`${currentView === 'coaching' ? 'text-[#f4c024]' : 'text-gray-700'}`}>
                <Video size={18} />
              </div>
              <div className="text-left">
                <div className={`text-[11px] font-black uppercase tracking-tight ${currentView === 'coaching' ? 'text-white' : 'text-gray-500'}`}>Mon Diagnostic Privé</div>
              ...
              {currentView === 'coaching' && <div className="ml-auto w-1.5 h-1.5 bg-[#f4c024] rounded-full" />}
            </button>
          </div>

          {activeModule.resources && currentView === 'module' && (
      ...
      <main className="flex-1 overflow-y-auto bg-black relative">
        <div className="max-w-4xl mx-auto px-8 py-20 lg:px-24 lg:py-32">
          {currentView === 'module' && renderModuleContent()}
          {currentView === 'coaching' && renderCoachingContent()}
          {currentView === 'qg' && <QGView />}
        </div>
      </main>
    </div>
  );
